...
if (!isset($_POST["cadastro"])){ //primeira visita do usuário
	...
	$msg[9] = "<label id=msg>Pelo menos 6 d&#237;gitos, compostos por letras e n&#250;meros</label>";
	$msg[10] = "";

	include("formularios/form-cadastro-cliente.php"); //formulário de cadastro
}
	...
	if($senha != null){ //validando senha e exibindo erro
		...
		elseif(validasenha($senha,$confirmasenha) == "pequena"){
			$msg[9] = "<label id=erro>(sua senha possui menos de 6 caracteres)</label>" ;
			$acheierro = TRUE;
		}
	...
	//exibindo erros
	if($acheierro == TRUE){ include("/formularios/form-cadastro-cliente.php");} 
	else{ 
...
criarusuario($_SESSION['email'],$_SESSION['senha'],$_SESSION['apelido'],'consumidor',$_SESSION['numreg']);

criarperfilusuario($_SESSION['numreg'],$_SESSION['nome_completo'],$_SESSION['apelido'],$_SESSION['cpf'],$_SESSION['email'],$_SESSION['telefone'],$_SESSION['endereco'],$_SESSION['complemento'],$_SESSION['cidade'],$_SESSION['estado'],$_SESSION['cep']); //criando perfil do usuário

//desconetardb(); //encerrando a conexão com a base de dados;
